...
### 6.1. IPv4와 IPv6 식별자
   "IPv6address"와 "IPv4address"를 위한 ABNF 규칙은 [RFC 3986]에 정의되어 있다.
   "IPv6address"는 텍스트 표현 권고[RFC 5952] 예를 들어 소문자나 0의 압축 등을 준수해야 한다.
   
   IP 주소는 [RFC 1918]과 [RFC 4193]에 정의된 바와 같이 내부 망으로부터의 것일 수 있다.
...
### 7.2. 헤더 필드 보존
   ...
   proxy가 304 응답에서 content가 불일치하는 경우를 감지하고, [RFC 7232]의 섹션 4.1을 따라 무조건 request를 반복하는 경우도 고려해야한다.
...
### 7.3. Relation to Via
   "Via" 헤더 필드([RFC 7230] 섹션 5.7.1)는 이 헤더 필드와 유사한 유스 케이스를 가진 헤더 필드이다.
